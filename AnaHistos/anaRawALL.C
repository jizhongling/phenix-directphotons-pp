...
void anaRawALL(const int process = 0)
{
  ...
  TTree *t1 = new TTree("t1", "A_{LL}");
  t1->Branch("runnumber", &runnumber, "runnumber/I");
  t1->Branch("spin_pattern", &spin_pattern, "spin_pattern/I");
  t1->Branch("crossing", &icr, "crossing/I");
  t1->Branch("ipt", &ipt, "ipt/I");
  t1->Branch("ALL", &ALL, "ALL/D");
  t1->Branch("eALL", &eALL, "eALL/D");

  TGraphErrors *gr[4*2*npT/ngroup];
  int igp[4*2*npT/ngroup] = {};
  ...
  while( fin >> runnumber )
  {
    ...
    for(int icr=0; icr<2; icr++)
      for(int ipol=0; ipol<3; ipol+=2)
      {
        ...
        for(int sector=0; sector<8; sector++)
          ...
          {
            int ih = sector + 8*icr + 2*8*ipol + 3*2*8*isolated + 2*3*2*8*evtype + 3*2*3*2*8*bbc10cm + 2*3*2*3*2*8*ival;
    ...
    for(icr=0; icr<2; icr++)
    {
      ...
      for(ipt=0; ipt<npT/ngroup; ipt++)
      {
        ...
        ALL = 1./(pb[0]*py[0]) * (npp - r*npm) / (npp + r*npm);
