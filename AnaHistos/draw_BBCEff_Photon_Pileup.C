...
void draw_BBCEff_Photon_Pileup()
{
  ...
  QueryTree *qt_pile = new QueryTree("data/BBCEff-photon-pileup.root", "RECREATE");

  QueryTree *qt_bbc = new QueryTree("data/BBCEff-photon.root");
  ...
  for(int part=0; part<2; part++)
    for(int ipt=0; ipt<npT; ipt++)
    {
      ...
      gr->Draw("AP");
      gr->SetTitle( Form("p_{T}: %3.1f-%3.1f GeV", pTbin[ipt], pTbin[ipt+1]) );
      gr->Fit(fn_pol1, "Q");
