  ...
  for(int beam=0; beam<3; beam++)
    for(int icr=0; icr<2; icr++)
    {
      ...
      for(int pttype=0; pttype<2; pttype++)
      {
        ...
        {
          int ic = beam + 3*isotype + 3*2*pttype + 3*2*2*icr;
          mc(ic, 4,4);
        ...
        {
          double nphoton = h_photon->GetBinContent(ipt+1);
          ...
          for(int isotype=0; isotype<2; isotype++)
          {
            ...
            int ptbin_first = h2_pion[isotype]->GetXaxis()->FindBin(pTbin_pol[ipt]);
            int ptbin_last = h2_pion[isotype]->GetXaxis()->FindBin(pTbin_pol[ipt+1]) - 1;
