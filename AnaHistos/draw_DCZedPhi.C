void draw_DCZedPhi()
{
  ...
  TH1 *h_zed_data = (TH1*)h3_dclive_data->ProjectionX("h_phi_data", 0,-1, 7,30)->Clone("h_zed_data");
  TH1 *h_phi_data = (TH1*)h3_dclive_data->ProjectionY("h_phi_data", 0,-1, 7,30)->Clone("h_phi_data");;
  ...
  TH1 *h_zed_sim = hn_dclive_sim->Projection(0);
  ...
  double scale_phi = h_phi_data->Integral(1,50) / h_phi_sim->Integral(1,50);
