TGraph* CreateGraph(TFile *f, Int_t arm)
{
  ...
  const Double_t Acceptance = 0.095;
  ...
  for(Int_t ipt=1; ipt<30; ipt++)
  {
    ...
    Double_t ndirect = nphoton - npion * ( 1 + 0.24 );
    gx[ipt] = (low + high) / 2.;
    gy[ipt] = BBCCross * (ndirect/BBCCount) / (2*PI*gx[ipt]) / ((high-low)*0.7) / Acceptance;

    cout << "pT=" << low << "-" << high << "\tnphoton=" << nphoton
      << "\tnpair=" << npair << "\tnfit=" << nfit << "\tnsub=" << nsub
      << "\tngpr=" << ngpr << "\tgy=" << gy[ipt] << endl;
  }
  ...
  TGraph *graph = new TGraph(gn, gx, gy);
  return graph;
}

void draw_DirectPhoton_Cross()
{
  ...
  for(Int_t arm=0; arm<2; arm++)
  {
    ...
    gr[arm]->SetTitle("DirectPhoton Cross Section");
    gr[arm]->GetXaxis()->SetTitle("p_{T} [GeV]");
    gr[arm]->GetYaxis()->SetTitle("pb*GeV^{-2}*c^{-3}");
    gr[arm]->GetXaxis()->SetRangeUser(0., 30.);
    gr[arm]->GetYaxis()->SetRangeUser(0.1, pow(10,5));
    gr[arm]->SetMarkerColor(arm+1);
    gr[arm]->SetMarkerStyle(arm+20);
    gr[arm]->SetMarkerSize(1.);
    if(arm == 0)
      gr[arm]->Draw("AP");
    else
      gr[arm]->Draw("P");
  ...
  leg->AddEntry(gr[0], "West arm", "P");
  leg->AddEntry(gr[1], "East arm", "P");
  leg->Draw();

  c0->Print("DirectPhoton-Cross.pdf");
}
