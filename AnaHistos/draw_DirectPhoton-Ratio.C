...
void draw_DirectPhoton_Ratio()
{
  ...
  for(Int_t arm=0; arm<2; arm++)
  {
    ...
    gr[arm]->SetTitle("DirectPhoton/Pi0Photon");
    gr[arm]->GetXaxis()->SetTitle("p_{T} [GeV]");
    gr[arm]->GetYaxis()->SetTitle("ratio");
    gr[arm]->GetXaxis()->SetRangeUser(0., 30.);
    gr[arm]->GetYaxis()->SetRangeUser(0., 3.);
  ...
  c0->Print("DirectPhoton-ratio.pdf");
}
