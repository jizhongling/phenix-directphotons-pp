...
void draw_IsoPionALL()
{
        ...
        for(int ipt=0; ipt<npT_pol; ipt++)
        {
          ...
          for(int icr=0; icr<2; icr++)
            for(int pattern=0; pattern<4; pattern++)
            {
              ...
              int igr = beam + 3*isotype + 3*2*pttype + 3*2*2*icr + 3*2*2*2*pattern + ngr;
          ...
          {
            int igr = beam + 3*isotype + 3*2*pttype + ngr/2*3;
  ...
  for(int beam=0; beam<3; beam++)
    for(int isotype=0; isotype<2; isotype++)
      for(int pttype=0; pttype<2; pttype++)
      {
        int ic = beam + 3*isotype + 3*2*pttype + ngr;
        mc(ic, 2,2);
        ...
        int igr = beam + 3*isotype + 3*2*pttype + ngr;
