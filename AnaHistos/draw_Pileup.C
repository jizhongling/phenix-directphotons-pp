        ...
        for(Int_t ic=0; ic<2; ic++)
          for(Int_t is=0; is<2; is++)
          {
            ...
            if( ic == 1 && ipt > 0 )
            {
              ...
              Double_t scale = sqrt( fn_mean->GetChisquare() / fn_mean->GetNDF() );
              Double_t mean = fn_mean->GetParameter(0);
              Double_t emean = fn_mean->GetParError(0) * scale;

              scale = sqrt( fn_fit[im]->GetChisquare() / fn_fit[im]->GetNDF() );
              Double_t p0 = fn_fit[im]->GetParameter(0);
              Double_t ep0 = fn_fit[im]->GetParError(0) * scale;

              Double_t xx = ( pTbin[id][ipt-1] + pTbin[id][ipt] ) / 2.;
              Double_t yy = p0 / mean;
              Double_t eyy = yy * sqrt( pow(emean/mean,2.) + pow(ep0/p0,2.) );
