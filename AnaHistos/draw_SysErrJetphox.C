...
void draw_SysErrJetphox()
{
  const char *jetphox_fname[5] = {"MMM", "MLL", "MLH", "MHL", "MHH"};
  //const char *jetphox_fname[5] = {"MMM", "LLH", "LHH", "HLL", "HHL"};
  ...
  for(int imu=0; imu<5; imu++)
  {
    ...
    TFile *f_nlo = new TFile( Form("data/isoprompt-x%d00-ct14-%s.root",imu==0?4:2,jetphox_fname[imu]) );
    ...
    for(int ipt=12; ipt<npT; ipt++)
    {
      ...
      if(imu == 0)
      {
        sigma_onept[ipt] = sigma_nlo/2.;
        esigma_onept[ipt] = esigma_nlo/2.;
      }
      else
      {
  ...
  c0->Print("plots/SysErrJetphox.pdf");
}
